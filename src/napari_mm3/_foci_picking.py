...
class FociPicking(MM3Container):
    ...
    def highlight_dividing(self):
        div_times, div_xs, div_ys = self.get_visible_cell_divisions()
        cur_first_frame = self.viewer.dims.current_step[1] + 1
        div_times -= cur_first_frame
        div_xs += div_times * self.img_width
        points = np.stack((div_ys, div_xs)).T
        self.viewer.add_points(points)


    def get_visible_cell_divisions(self):
        division_times = []
        division_xs = []
        division_ys = []
        for cell_id, cell in self.cells.items():
            division_times.append(cell.division_time - 1)
            division_y, division_x = cell.centroids[-1]
            division_xs.append(division_x)
            division_ys.append(division_y)
        
        return np.array(division_times), np.array(division_xs), np.array(division_ys)

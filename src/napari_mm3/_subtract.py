...
from ._function import information, warning, load_stack, load_specs
...
def subtract_fov_stack(params, fov_id, specs, color='c1', method='phase'):
    ...
    for peak_id in ana_peak_ids:
        ...
        if method == 'phase':
            ...
            subtracted_imgs = pool.map(subtract_phase_helper, subtract_args, chunksize=10)
        elif method == 'fluor':
            subtract_args = [(params, pair[0], pair[1]) for pair in subtract_pairs]
            subtracted_imgs = pool.map(subtract_fluor_helper, subtract_args, chunksize=10)

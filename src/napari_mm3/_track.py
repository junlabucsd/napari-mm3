...
from ._function import information, load_specs, Cell, load_stack, load_time_table, find_complete_cells, plot_lineage_images, find_cells_of_birth_label
        ...
        if not cell_leaves:
            for region in regions:
                if region.centroid[0] < new_cell_y_cutoff and region.label <= new_cell_region_cutoff:
                    ...
                    Cells[cell_id] = Cell(time_table, cell_id, region, t, parent_id=None)
            ...
            for leaf_id, region_links in six.iteritems(leaf_region_map):
                ...
                # there may be two daughters, or maybe there is just one child and a new cell
                    ...
                    if check_division_result == 3:
                        ...
                        Cells[daughter1_id] = Cell(time_table, daughter1_id, region1, t,
                                                   parent_id=leaf_id)
                        Cells[daughter2_id] = Cell(time_table, daughter2_id, region2, t,
                                                   parent_id=leaf_id)
                    ...
                    # The other region should be a leaf it passes the requirements
                        ...
                        if region2.centroid[0] < new_cell_y_cutoff and region2.label <= new_cell_region_cutoff:
                            ...
                            Cells[cell_id] = Cell(time_table, cell_id, region2, t, parent_id=None)
                            cell_leaves.append(cell_id) # add to leaves
                    ...
                    # ditto for 2
                        ...
                        if region1.centroid[0] < new_cell_y_cutoff and region1.label <=     new_cell_region_cutoff:
                            ...
                            Cells[cell_id] = Cell(time_table, cell_id, region1, t, parent_id=None)
                            cell_leaves.append(cell_id) # add to leaves
...
def Lineage(params):
    ...
    fig, ax = plot_lineage_images(params, Cells, fov_id_d, peak_id_d, Cells2,
...
def track_update_params(experiment_name, experiment_directory, image_directory, FOV, phase_plane, pxl2um, lost_cell_time, new_cell_y_cutoff, new_cell_region_cutoff, max_growth_length, min_growth_length, seg_img):
    params=dict()
...
def Track(experiment_name: str='', experiment_directory= Path(), image_directory:str='TIFF/',

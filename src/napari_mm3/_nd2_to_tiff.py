...
def nd2ToTIFF(experiment_directory : Path, tif_dir : str, tif_compress : int, image_start : int, image_end : int, vertical_crop = None, tworow_crop = None, fov_list = []):
    '''
    ...
    for nd2_file in nd2files:
        ...
        with pims_nd2.ND2_Reader(nd2_file) as nd2f:
            ...
            extraction_range = range(image_start,
                                     image_end + 1)
                ...
                for fov_id in range(0, nd2f.sizes[u'm']): # for every FOV
                    ...
                    if vertical_crop or tworow_crop:
                        ...
                        if tworow_crop:
                            ...
                            image_data_one = image_data[:,tworow_crop[0][0]:tworow_crop[0][1],:]
                            tif_filename = file_prefix + "_t%04dxy%02d_1.tif" % (t, fov)
                            information('Saving %s.' % tif_filename)
                            tiff.imsave(tif_dir / tif_filename, image_data_one, description=metadata_json, compress=tif_compress, photometric='minisblack')

                            # cut and save bottom row
                            metadata_t['fov'] = fov # update metdata
                            metadata_json = json.dumps(metadata_t)
                            image_data_two = image_data[:,tworow_crop[1][0]:tworow_crop[1][1],:]
                            tif_filename = file_prefix + "_t%04dxy%02d_2.tif" % (t, fov)
                            information('Saving %s.' % tif_filename)
                            tiff.imsave(tif_dir / tif_filename, image_data_two, description=metadata_json, compress=tif_compress, photometric='minisblack')
                        ...
                        # for just a simple crop
                            ...
                            information('Saving %s.' % tif_filename)
                            tiff.imsave(tif_dir / tif_filename, image_data, description=metadata_json, compress=tif_compress, photometric='minisblack')
...
def Nd2ToTIFF(experiment_directory = Path(), image_directory = "TIFF", image_start : int= 1, image_end: int = 50, FOVs_range: str = ""):
    ...
    nd2ToTIFF(experiment_directory, tif_dir, number_of_rows=1, tif_compress=5, image_start=image_start, image_end=image_end, crop_ymin = None, crop_ymax = None, fov_list = fov_list, tworow_crop = None)
    ...
    fovs =  list(sorted(set(int(fov_regex.search(filename).group()[2:]) for filename in image_name_list)))
    if fov_list:
        fovs = fov_list
